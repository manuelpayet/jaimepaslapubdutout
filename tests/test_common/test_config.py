...
class TestRadioListenerConfig:
    ...
    def test_default_values(self):
        ...
        config = RadioListenerConfig(rtsp_url="rtsp://example.com/stream")

        assert config.rtsp_url == "rtsp://example.com/stream"
    ...
    def test_validation_block_duration(self):
        ...
        with pytest.raises(ValueError, match="block_duration must be positive"):
            RadioListenerConfig(rtsp_url="rtsp://test.com", block_duration=0)

        with pytest.raises(ValueError, match="block_duration must be positive"):
            RadioListenerConfig(rtsp_url="rtsp://test.com", block_duration=-10)

    def test_validation_sample_rate(self):
        """Test validation of sample_rate."""
        with pytest.raises(ValueError, match="sample_rate must be positive"):
            RadioListenerConfig(rtsp_url="rtsp://test.com", sample_rate=0)
    ...
    def test_validation_whisper_model(self):
        ...
        with pytest.raises(ValueError, match="Invalid whisper_model"):
            RadioListenerConfig(rtsp_url="rtsp://test.com", whisper_model="invalid")
...
class TestConfigLoader:
    ...
    def test_load_from_kwargs(self):
        ...
        config = ConfigLoader.load_radio_listener_config(
            rtsp_url="rtsp://test.com/stream", block_duration=20, whisper_model="small"
        )
